...
function complexConvert(data) {
  ...
  let parse = JSON.stringify(res, (key, value) => {
...
function manipulateUrl(url) {
  ...
  let res = new URL(url);
  res.protocol = "https";
  res.host = "newhost.com";
  res.searchParams.set("newParam", "newValue");
  res.searchParams.delete("oldParam");
  ...
  return {
    ...
    search: res.search,
    params: res.searchParams.entries(),
  };
}
...
function searchParamsURL(url) {
  ...
  let res = new URL(url).searchParams;
  let params = new Map();
...
function checkURLParameters(url, params) {
  ...
  for (let value of params) {
    res[value] = urlObj.searchParams.has(value)
  }
    return res;
}
...
function processUrl(url, options) {
  ...
  if(options.searchParams) {
    let res = options.searchParams;
    for(let par in res) {
      urlObj.searchParams.append(par, res[par])
    }
  }
    if(options.protocol) {
...
function manipulateQuery(url, options) {
  ...
  if(options.has("append")) {
    let app = options.get("append");
    for(let [key, value] of app) {
      urlObj.searchParams.append(key, value)
    }
  }
  if(options.has("delete")) {
    let del = options.get("delete");
    for(let par of del) {
      urlObj.searchParams.delete(par)
    }
  }
  return urlObj.toString();
}
...
function sortUrlParams(url) {
  ...
  let urlObj = new URL(url);
  let res = Array.from(urlObj.searchParams.entries());
  res.sort((a, b) => a - b);
...
function getURLValues(url) {
  ...
  let mas = Array.from(URLSearchParams.keys());
  let res = [];
  for(let key of mas) {
    let item = URLSearchParams.getAll(key);
    res.push(... item);
  }
  return res;
}
